...
function initDropDownGos() {
	...
	$(document).click(function (event) {
		if ($(event.target).closest(".wrapper-gos-site.active").length) return;
...
function initDropDownLang() {
	...
	$(document).click(function (event) {
		if ($(event.target).closest(".block-dropdown_lang.active").length) return;
...
function initDropDownSoc() {
	...
	$(document).click(function (event) {
		if ($(event.target).closest(".wrapper-link-social-network.active").length) return;
...
function initScrollHeader() {
	...
	$(window).scroll(function (event) {
		...
		if (st > lastScrollTop) {
			...
			$('.container-wrapper-header').css((_$$css = {
				"background": "-moz-linear-gradient(top, rgba(21,46,98,0.9) 0%, rgba(18,51,119,0.4) 50%, rgba(18,51,119,0) 100%)"
			}, _defineProperty(_$$css, "background", "-webkit-linear-gradient(top, rgba(21,46,98,0.9) 0%,rgba(18,51,119,0.4) 50%,rgba(18,51,119,0) 100%)"), _defineProperty(_$$css, "background", "linear-gradient(to bottom, rgba(21,46,98,0.9) 0%,rgba(18,51,119,0.4) 50%,rgba(18,51,119,0) 100%)"), _defineProperty(_$$css, "filter", "progid:DXImageTransform.Microsoft.gradient( startColorstr='#e6152e62', endColorstr='#00123377',GradientType=0 )"), _$$css));
			$('.block-header-logo').css({
				"width": "78px"
			});
...
function initMenu() {
	...
	$('.js-menu-category .menu_categories li a').on('mouseover', function (e) {
		...
		clearSetTimeout = setTimeout(function () {
			...
			if ($(__self).data('index') == undefined) _e.preventDefault();else {
	...
	function FixHeaderCol(tbl, fixRows, fixCols, ww, hh) {
		...
		for (i = 0; i < rowsCnt; i++) {
			...
			for (j = 0; j < td.length; j++) {
				...
				if (i < fixRows) {
					//ну и заодно фиксируем заголовок
